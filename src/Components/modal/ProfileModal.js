...
function ProfileModal({ isProfileModelOpen, setIsProfileModelOpen }) {
  const { user_id, username, email, full_name, bio, image } = useSelector(
    (state) => state.auth
  );
  ...
  const handleSaveChanges = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://127.0.0.1:8000/chat/edit/${user_id}/`,
        formData,
        {
          headers: {
        ...
        <div className="h-[37rem] bg-white m-3 flex flex-col justify-center items-center">
          <div className="relative w-[10rem] h-[10rem] my-4 bg-slate-200 rounded-full">
            {image ? (
              ...
              <img
                src="https://dreamschat.dreamstechnologies.com/react/template/assets/img/avatar/avatar-2.jpg"
                alt="image"
                className="rounded-full w-full h-full object-cover"
              />
            )}
            <button
              className="absolute bottom-0 border-4 border-white right-1 transform rounded-full -translate-x-1/2 bg-[#420BA1] text-white p-2 text-2xl"
          ...
          <div>
            ...
            <div className="flex justify-end mt-9">
              <Button
                className="bg-[#420BA1] text-white m-2"
                variant="text"
                onClick={handleClose}
              >
                Cancel
