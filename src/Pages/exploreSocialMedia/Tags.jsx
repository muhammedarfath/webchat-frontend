    ...
    const fetchTagPosts = async () => {
      try {
        const response = await axios.get(
          `http://127.0.0.1:8000/posts/tag-post/${title}/`
        );
        if (response.data) {
      ...
      {!loading && (
        <div className="w-full flex flex-col overflow-auto">
          <div className="w-full h-1/2 flex-row flex gap-1 p-5">
            {tagpost.length > 0 ? (
              <img
                src={`http://127.0.0.1:8000${getRandomImage()}`}
                alt="img"
                className="rounded-full border-1 lg:w-52 lg:h-52 w-28 h-28 object-cover"
              />
            ) : (
              <img
            ...
            <div className="flex flex-col justify-center gap-2 ml-5">
              ...
              <small className="text-xl">
                {tagpost.length.toLocaleString()} posts
              </small>
            </div>
          </div>
